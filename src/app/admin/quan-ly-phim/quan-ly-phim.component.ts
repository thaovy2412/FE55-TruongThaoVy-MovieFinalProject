...
export class QuanLyPhimComponent implements OnInit {
  @ViewChild('btnShowAddMovieSuccessModal') btnShowAddMovieSuccessModal: ElementRef;
  @ViewChild('btnShowDelMovieSuccessModal') btnShowDelMovieSuccessModal: ElementRef;
  ...
  formAddMovie: FormGroup = new FormGroup({
    ...
    danhGia: new FormControl('',[Validators.required, Validators.pattern('^([1-9]|10)$')]),
  });
  ...
  formAddShowtimes: FormGroup = new FormGroup({
    ...
    giaVe: new FormControl('',[Validators.required,Validators.max(200000), Validators.min(75000)]),
  });
  ...
  handleAdd() {
    ...
    this.loading=true;
    this.error="";
    let date:string = this.getDate(this.formAddMovie.controls['ngayKhoiChieu'].value);
  ...
  handleEdit() {
    ...
    this.loading=true;
    this.error= "";
    // let str: string = this.formEditMovie.controls['ngayKhoiChieu'].value;
    // let date: string = str.replace(/-/g, '/');
    // let year: string = date.slice(0, 4);
    // let month: string = date.slice(5, 7);
    // let day: string = date.slice(8, 10);
    // let ngaykc: string = day + '/' + month + '/' + year;
    let date: string= this.getDate(this.formEditMovie.controls['ngayKhoiChieu'].value)
  ...
  getMovie(phim: Phim) {
    ...
    this.movieService.getInfoMovieShowTimes(phim.maPhim).subscribe({
      next: (result) => {
        ...
        console.log('ttlc', this.thongTinLichChieuPhim);
      },
    });
  }
  ...
  handleAddShowTimes() {
    ...
    console.log(this.formAddShowtimes.value);
    this.adminService.taoLichChieu(this.formAddShowtimes.value).subscribe({
      ...
      error: (err) => {
        ...
        if (err.error.text) {
          ...
          this.formAddShowtimes.controls['maPhim'].setValue(this.thongTinLichChieuPhim.maPhim);
          console.log(this.formAddShowtimes.value);
          this.movieService.getInfoMovieShowTimes(this.formAddShowtimes.controls['maPhim'].value).subscribe({
            next: (showtimes) => {
  ...
  getDate(datetime:Date){
    ...
    return this.padLeadingZeros(date.getDate(),2)+'/'+this.padLeadingZeros(date.getMonth()+1,2)+'/'+this.padLeadingZeros(date.getFullYear(),2);
  }
  getDateTime(date:Date){
    let datetime = new Date(date);
    return this.padLeadingZeros(datetime.getDate(),2)+'/'+this.padLeadingZeros(datetime.getMonth()+1,2)+'/'+this.padLeadingZeros(datetime.getFullYear(),2)+' '+this.padLeadingZeros(datetime.getHours(),2)+':'+this.padLeadingZeros(datetime.getMinutes(),2)+':'+this.padLeadingZeros(datetime.getSeconds(),2);
  }
  reload(){
